...
```
                 ...
                 │
RTK (base_link) ─┼──→ T_imu_base ──→ ESKF Update(GPS) ──→ 约束 imu_link 状态
                 │
LiDAR (lidar)  ──┼──→ T_imu_lidar ──→ ICP ──→ 帧间匹配 ✅
          ...
          PGO 优化 (imu_link) ✅
                          │
                          ▼
   输出: T_base_imu * pose_imu → base_link pose → TF 发布
```
...
| RTK→ESKF | base_link 当 imu_link 用 | `T_imu_base * RTK_base → RTK_imu` | 新增变换 |
| 位姿发布 | imu_link 标成 base_link 发 | `T_base_imu * pose_imu → pose_base` | 新增变换 |
...
| 文件 | 改动类型 | 说明 |
|------|----------|------|
| `common/rtk_utils.h/cc` | 修改 | `RTKConverter::Convert()` 中新增 `T_imu_base` 变换 |
| `core/localization/localization_result.h/cc` | 修改 | 输出时 `T_base_imu * pose_imu → pose_base` |
| `config/default*.yaml`（6 个） | 修改 | 新增 `common.T_imu_base` |

#### 13.8.2 改动 A：rtk_utils.h — RTKConverter 新增 T_imu_base
...
class RTKConverter {
   ...
   private:
    SE3 T_imu_base_ = SE3();  // 默认 Identity，向后兼容
};
```

#### 13.8.3 改动 B：rtk_utils.cc — Convert() 中新增变换

```cpp
// IMU→base_link 变换：将 RTK 位姿从 base_link 转到 imu_link
if (!T_imu_base_.matrix().isIdentity(1e-10)) {
    rtk.position = T_imu_base_ * rtk.position;
    // 姿态不变（base_link 和 imu_link 安装时对齐，无旋转差异）
}
```

> **注意**：双天线 INS 输出的是 ENU 绝对姿态，安装时 imu_link 和 base_link 的坐标系方向一致（都由 INS 模块标定对齐），所以 `T_imu_base` 的旋转部分为单位阵，只有平移分量（x/y/z 杆臂）。

#### 13.8.4 改动 C：localization_result.cc — 输出侧变换

```cpp
// 位姿输出时：imu_link → base_link 变换
pose_output = T_imu_base_.inverse() * pose_imu;
```
...
```yaml
common:
  T_imu_base: [0.0, 0.0, 0.0]  # IMU→base_link 杆臂 (m)，默认零杆臂，向后兼容
```
...
| T_imu_base 只有平移 | 双天线 INS 已标定 imu_link 和 base_link 方向对齐，旋转 = Identity |
| 默认零杆臂 | 向后兼容现有配置，不设 `T_imu_base` 时行为不变 |
| ESKF 状态不动 | 只在观测输入和位姿输出两个边界做变换，核心算法完全透明 |
| 改动集中在 RTKConverter | 单一入口，SLAM 和定位模式同时受益 |
